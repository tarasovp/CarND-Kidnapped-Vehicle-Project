...
//coordinat transformer
inline LandmarkObs coordinate_transform(const Particle p, const LandmarkObs o )
{
    return LandmarkObs{ o.id, cos(p.theta)*o.x - sin(p.theta)*o.y + p.x,
        sin(p.theta)*o.x + cos(p.theta)*o.y + p.y };
    
}

//weight transformer
inline double GetWeight (const LandmarkObs l1,  const LandmarkObs l2,
                         const double a, const double b, const double c)
{
    return a * exp( -( pow(l1.x-l2.x,2)/b + (pow(l1.y-l2.y,2)/c)) );;
    
}

inline bool range_filter(const Particle p, const LandmarkObs o, const double sensor_range )
{
    return dist(o.x,o.y,p.x, p.y)<=sensor_range;
}
